...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (songLowercase)
		{
			...
			case 'donut':
				dialogue = CoolUtil.coolTextFile(Paths.txt('data/donut/donutDialogue'));
			case 'galactic':
				dialogue = CoolUtil.coolTextFile(Paths.txt('data/galactic/galacticDialogue'));
			case 'headache':
				dialogue = CoolUtil.coolTextFile(Paths.txt('data/headache/headacheDialogue'));
			case 'cutscene':
				dialogue = CoolUtil.coolTextFile(Paths.txt('data/cutscene/cutsceneDialogue'));
				isEmptySong = true;
				...
				case 'alley':
					{
						defaultCamZoom = 0.8;
						curStage = 'alley';
						var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('alley', 'manic'));
						if(FlxG.save.data.antialiasing)
						{
							bg.antialiasing = true;
						}
				...
				case 'space':
					{
						defaultCamZoom = 0.8;
						curStage = 'space';
						var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('space', 'manic'));
						if(FlxG.save.data.antialiasing)
						{
							bg.antialiasing = true;
						}
				...
				case 'room':
					{
						defaultCamZoom = 0.8;
						curStage = 'room';
						var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('room', 'manic'));
						if(FlxG.save.data.antialiasing)
						{
							bg.antialiasing = true;
						}
		...
		switch (SONG.player2)
		{
			...
			case 'kiki':
				dad.x -= 850;
				dad.y -= 510;
				camPos.set(dad.getMidpoint().x + 900, dad.getMidpoint().y + 600);
			case 'kikiali':
				dad.x -= 850;
				dad.y -= 435;
				camPos.set(dad.getMidpoint().x + 900, dad.getMidpoint().y + 600);
	...
	function schoolIntro(?dialogueBox:DialogueBox):Void
	{
		...
		if(isEmptySong)
		{
			var blacker:FlxSprite = new FlxSprite(-500, -500).makeGraphic(FlxG.width * 2, FlxG.height * 2, FlxColor.BLACK);
			blacker.scrollFactor.set();
			add(blacker);
		}
			...
			if (camFollow.x != dad.getMidpoint().x + 150 && !PlayState.SONG.notes[Std.int(curStep / 16)].mustHitSection)
			{
				...
				switch (dad.curCharacter)
				{
					...
					case 'kiki':
						...
						camFollow.x = dad.getMidpoint().x + 900;
					case 'kikiali':
						camFollow.y = dad.getMidpoint().y + 600;
						camFollow.x = dad.getMidpoint().x + 900;
					case 'crash':
			...
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					if(SONG.song.toLowerCase() == "headache")
					{
						...
						FlxG.switchState(new WinScreenState());
					}
				...
				else
				{
					...
					{
						case 'headache':
							LoadingState.loadAndSwitchState(new VideoState("assets/videos/cass/manicVid2.webm", new PlayState()));
						default:
